...
import { quickDefine, filterOCRWords } from '../services/geminiService';
...
const OCRView: React.FC<OCRViewProps> = ({ onBack }) => {
  ...
  const processImage = async (dataUrl: string) => {
    ...
    try {
      ...
      const uniqueRawWords: string[] = Array.from(new Set(rawWords));
      
      // AI 智能筛选
      if (uniqueRawWords.length > 0) {
        // 临时显示原始结果，避免等待过久
        setTokens(uniqueRawWords.slice(0, 20)); 
        
        const validWords = await filterOCRWords(uniqueRawWords);
        if (validWords.length > 0) {
          setTokens(validWords);
        } else {
          // AI 失败或过滤后为空，回退到原始结果
          setTokens(uniqueRawWords);
        }
      } else {
        setTokens([]);
      }
  ...
  return (
    <div className="min-h-screen bg-white dark:bg-darkBg p-6 pt-12 animate-fade-in flex flex-col transition-colors">
      ...
      {!image ? (
        ...
        <div className="flex-1 overflow-y-auto space-y-6">
          <div className="relative rounded-3xl overflow-hidden shadow-lg aspect-video bg-black/5 dark:bg-black/40 border dark:border-slate-800">
             ...
             {isProcessing && (
                 <div className="absolute inset-0 bg-white/80 dark:bg-darkBg/80 backdrop-blur-sm flex flex-col items-center justify-center p-6">
                    ...
                    <span className="text-sm font-black text-slate-900 dark:text-white uppercase tracking-widest animate-pulse">
                      {progress < 1 ? `Scanning • ${Math.round(progress * 100)}%` : 'AI Filtering...'}
                    </span>
                 </div>
             )}
          </div>
          ...
          <div className="bg-white dark:bg-darkSurface rounded-[2.5rem] border border-gray-100 dark:border-slate-800 p-8 min-h-[200px] text-left">
            <h3 className="text-[10px] font-black text-gray-400 dark:text-gray-600 uppercase tracking-[0.2em] mb-6">
              {isProcessing && progress >= 1 ? 'AI 正在智能筛选高频词...' : '识别出的单词词元 (点击收藏)'}
            </h3>
