...
const WordSearch: React.FC<WordSearchProps> = ({ currentDatasetId, onBack }) => {
  ...
  return (
    <div className="min-h-screen bg-slate-50 dark:bg-darkBg flex flex-col animate-slide-up transition-colors">
      <div className="p-6 pt-12 max-w-2xl w-full mx-auto">
        <header className="flex items-center gap-4 mb-8">
            ...
            <h2 className="text-2xl font-black text-slate-900">智能搜词</h2>
        </header>
        ...
        <form onSubmit={handleSearch} className="relative mb-10">
            <input 
              ...
              className="w-full bg-white px-6 py-5 rounded-[2rem] border-2 border-slate-100 focus:border-primary outline-none text-lg font-medium shadow-xl shadow-slate-200/50 transition-all pr-20"
            />
        ...
        {result && (
          <div className="bg-white p-8 rounded-[2.5rem] shadow-xl border border-slate-50">
            <div className="flex justify-between items-start mb-4">
              <div>
                <h3 className="text-4xl font-black text-slate-900">{result.word}</h3>
                <p className="text-primary font-mono font-bold mt-1">{result.phonetic}</p>
              </div>
              <span className={`text-[10px] font-bold px-3 py-1 rounded-full uppercase tracking-tighter ${matchStatus === 'local_match' ? 'bg-emerald-50 text-emerald-600' : 'bg-amber-50 text-amber-600'}`}>
                {matchStatus === 'local_match' ? '已在库中' : matchStatus === 'local_diff_tag' ? '全库匹配' : 'AI 释义'}
              </span>
            </div>
            
            <p className="text-2xl font-bold text-slate-800 mb-4">{result.translation}</p>
            <p className="text-slate-500 italic leading-relaxed mb-10 border-l-4 border-slate-100 pl-4">"{result.example}"</p>

            {matchStatus !== 'local_match' && (
               <button onClick={handleAdd} disabled={isAdding} className="w-full py-4 bg-slate-900 text-white rounded-2xl font-bold hover:scale-[1.02] transition-transform">
                 {isAdding ? '添加中...' : `将 "${result.word}" 加入 ${currentDatasetId}`}
               </button>
            )}
            
            {matchStatus === 'local_match' && (
              <div className="w-full py-4 bg-emerald-50 text-emerald-600 rounded-2xl font-bold text-center">
                单词已在您的学习清单中
              </div>
            )}
          </div>
